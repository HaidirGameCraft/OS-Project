...
void page_initialize() {
    ...
    for(int i = 0; i < size_page; i++) {
        u32 addr = ( krlstart & ~0xFFF ) + i * PAGE_SIZE;
        
        first_table.entries[ PT_INDEX( addr ) ] = addr | PAGE_ATTR_PRESENT | PAGE_ATTR_READWRITE;
    }
    ...
    extern struct page_table_t video_memory_table;
